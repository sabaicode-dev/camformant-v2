"use client";
import { EducationParams } from "@/app/self/(cv-rating)/educ/page";
import { ExperienceParams } from "@/app/self/(cv-rating)/exp/page";
        ...
        if (submitEnabled) {
            ...
            if(setValue){
                if(mode=="date")
                    ...
                    setValueArray&&setValueArray((prev: EducationParams[]|ExperienceParams[]|any[]) => {
        ...
        <div className="container w-full h-full flex flex-col justify-between">
                    ...
                    <>
                     <div>{index}</div>
                        <input
                            ...
                            value={startYear}
                            onChange={(e) => setStartYear && setStartYear((prev:string[])=>{
                                const previous=[...prev]
                        ...
                        <input
                            ...
                            className="w-60 p-5 outline-none shadow-md rounded-xl"
                            value={endYear}
                            onChange={(e) => setEndYear && setEndYear((prev:string[])=>{
                                const previous=[...prev]
                                previous[index as number]=e.target.value
                                return previous
                            })}
            ...
            <div className="pb-10">
                <button
                    ...
                    className={`p-5 ${submitEnabled ? "bg-primary" : "bg-slate-300"} w-full rounded-2xl text-white`}
                >
                    Done
                </button>
            </div>
